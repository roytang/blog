...
<span class="ui_qtext_rendered_qtext"><p class="ui_qtext_para u-ltr u-text-align--start">When developing new features, make sure to write to clean, maintainable code that future maintainers won't have a problem with. If you can write unit tests, do so.</p><p class="ui_qtext_para u-ltr u-text-align--start">When maintaining old features, it's kind of like being a doctor: first, do no harm. Perform the minimum changes necessary to implement your changes to minimize the chance of breaking existing behavior. But still try to avoid writing terrible code. It helps if the existing feature has unit tests</p></span>