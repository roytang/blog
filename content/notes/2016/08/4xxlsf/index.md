---
date: 2016-08-16 11:55:08
source: reddit
syndicated:
- type: reddit
  url: https://reddit.com/r/ipad/comments/4xxlsf/whats_a_good_app_to_use_with_ipad_pro_pencil_to/
tags:
- ipad
---

#### Posted on r/ipad: [What's a good app to use with iPad Pro + Pencil to make comic strips?](https://reddit.com/r/ipad/comments/4xxlsf/whats_a_good_app_to_use_with_ipad_pro_pencil_to/)

I've been using Sketchbook Pro since that's what I use for my sketching, but I find that my hands are very uneven at drawing the speech bubbles, so I'm hoping for something that includes a smoothing process for the lines, and also a way to input multiple lines of text easily